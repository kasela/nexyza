# Nexyza Enhancement Roadmap

This repo includes a first enhancement bundle focused on the existing analyser experience while preserving the current dark glass theme.

## Completed in this build

1. Stateful analyser tabs
   - Active tab is restored from URL hash or local storage.
2. Remembered chart workspace state
   - Search text, active type filter, layout, and sort order are restored per upload.
...
4. Keyboard shortcuts
   - `P` present mode
   - `/` focus chart search
   - `1-7` switch analyser tabs
5. Chart grid quality-of-life
   - A visible chart summary
   - Sort by title ascending, descending, or by chart type

## Recommended next development phases

### Phase 1 — Stabilize analyser shell
- Move large inline JavaScript from `templates/analyser/result.html` to static JS modules
- Add regression tests for tabs, fullscreen, present mode, and chart actions
- Standardize UUID-safe handlers and DOM attributes

### Phase 2 — Improve chart persistence
- Save chart display preferences server-side
- Add saved chart presets
- Add more chart builder controls and formatting options

### Phase 3 — Report and export maturity
- Improve PPTX and PDF fidelity
- Add scheduled reports and delivery history
- Add boardroom presentation mode options and notes

### Phase 4 — Data preparation workflow
- Cleaner history and undo
- Join diagnostics and unmatched-row preview
- Formula templates and validation
- Reusable pipelines

### Phase 5 — AI and collaboration
- Better NLQ follow-up suggestions
- Insight style presets
- Comments, mentions, and approvals
